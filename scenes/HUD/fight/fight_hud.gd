...
func confirm_menu():
        if Input.is_action_just_pressed("ui_accept"):
                if pointer_index >= current_fight_menu_list.size():
                        return
                ...
                if next_lv_menu_size != null && next_lv_menu_size != 0:
                        # 先清空旧按钮
                        for child in fight_menu_next_lv.get_children():
                                child.queue_free()
                        button_next_lv_list.clear()
...
func select_menu():
        ...
        if current_button_list.is_empty():
                return
        ...
        if pointer_index >= 0 and pointer_index < current_button_list.size():
