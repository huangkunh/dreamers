...
class TankData:
        ...
        var c_device: Dictionary = {}       ## C装置 (战斗辅助)
        # 状态
...
## 装备战车配件
func equip_part(tank_id: String, equip_data: Dictionary) -> bool:
        if not tanks.has(tank_id):
                return false
        var tank = tanks[tank_id]
        match equip_data.get("type", -1):
                0: # MAIN_CANNON
                        if not tank.main_cannon.is_empty():
                                unequip_part(tank_id, 0)
                        tank.main_cannon = equip_data
                        tank.attack = equip_data.get("attack", tank.attack)
                        tank.max_ammo = equip_data.get("ammo_max", tank.max_ammo)
                        tank.current_ammo = tank.max_ammo
                1: # SUB_WEAPON
                        if not tank.sub_weapon.is_empty():
                                unequip_part(tank_id, 1)
                        tank.sub_weapon = equip_data
                2: # ENGINE
                        if not tank.engine.is_empty():
                                unequip_part(tank_id, 2)
                        tank.engine = equip_data
                        tank.speed = equip_data.get("speed", tank.speed)
                3: # ARMOR
                        if not tank.armor.is_empty():
                                unequip_part(tank_id, 3)
                        tank.armor = equip_data
                        tank.defense = equip_data.get("defense", tank.defense)
                        tank.max_hp += equip_data.get("max_hp_bonus", 0)
                4: # C_DEVICE
                        if not tank.c_device.is_empty():
                                unequip_part(tank_id, 4)
                        tank.c_device = equip_data
        print("[TankSystem] %s 装备了 %s" % [tank.name, equip_data.get("name", "???")])
        return true

## 卸下战车配件
func unequip_part(tank_id: String, slot: int) -> Dictionary:
        if not tanks.has(tank_id):
                return {}
        var tank = tanks[tank_id]
        var old: Dictionary = {}
        match slot:
                0: old = tank.main_cannon; tank.main_cannon = {}
                1: old = tank.sub_weapon; tank.sub_weapon = {}
                2: old = tank.engine; tank.engine = {}
                3: old = tank.armor; tank.armor = {}
                4: old = tank.c_device; tank.c_device = {}
        return old

## 获取C装置技能
func get_c_skill(tank_id: String) -> Dictionary:
        if not tanks.has(tank_id):
                return {}
        var tank = tanks[tank_id]
        if tank.c_device.is_empty():
                return {}
        return tank.c_device

## 修复战车 (消耗金币)
func repair_tank(tank_id: String) -> int:
        if not tanks.has(tank_id):
                return 0
        var tank = tanks[tank_id]
        var cost = (tank.max_hp - tank.current_hp) * 2  # 每HP 2G
        tank.current_hp = tank.max_hp
        print("[TankSystem] %s 修复完成, 花费 %dG" % [tank.name, cost])
        return cost
